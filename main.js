...
async function main(userlandRW, wkOnly = false) {
    ...
    // البحث عن واجهة الشبكة وتحديث حالة الاتصال
    ...
    // تحديث حالة الاتصال في الواجهة الجديدة
    let statusText = document.getElementById("status-text");
    let statusIcon = document.getElementById("status-icon");
    if (statusText && statusIcon) {
        if (ip.name === "Offline") {
            statusText.innerText = "Status: Offline (Local)";
            statusIcon.style.color = "#ff5555";
        } else {
            statusText.innerText = `Status: Online (${ip.ip}) - ${ip.name}`;
            statusIcon.style.color = "#55ff55";
        }
    }

    // إزالة الكلاسات القديمة وإضافة الجديدة للكونسول
    ...
    // @ts-ignore
    document.getElementById('status-text').innerHTML = `Listening: <span class="fw-bold">${ip.ip}</span> (Ports: ${ports}) (${ip.name})`;
    ...
    // عرض إشعارات الحالة بشكل حديث
    if (ip.name === "Offline") {
        showToast("📡 Offline Mode", 4000);
    } else if (ip.name === "wlan0") {
        showToast("📶 Wi-Fi Connected (wlan0)", 4000);
    } else if (ip.name === "eth0") {
        showToast("🔌 Ethernet Connected (eth0)", 4000);
    }
